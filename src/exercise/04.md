...
- You can pass a node generating function into the React.createElement()
- Use the children prop to state what is inside the HTML object.
- React components NEED to be capitalized (first letter) for JSX to know the difference between a React component vs. HTML element. 
- Use 'PropTypes' to enforce type on props during runtime as they get passed to components
- Use 'typeof' to find the data type
- Can use an external propTypes library to check prop types against
- Use React Fragments (<> </>) for quick nodes that don't need to be specified. (ex. providing a parent div for JSX)
