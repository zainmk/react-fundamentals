...
- Add the id to the form 'input' element to reference the value later from 'event.target.elements'.
- 'ref' will keep the state constant between react renders. Changing a ref does not trigger a re-render. (silent updaets ~ useful to preserve the visual of the app). Don't read while rendering.
- Use the 'ref' attribute in HTML DOM's to assign the ref to the HTML value. (instead of using ref.current)
- 'useState' is a responsive state variable that as it updates, rerenders the react app
- You can 'link' the useState variable to the HTML node through its value attribute. (initialize with the right variable when linked this way - ex. an empty string for string inputs)
