...
- React renders a list of HTMl elements, when using an array of values, however adding to the list dynamically does not always preserve the order of where it is inserted.
- The 'key' prop is how React knows the unique object either added/removed and where that uniqueness falls in the order. The key must also be unique within the list of items. 
- You can manipulate the keys to reset the state in this manner as well.
