...
- Recall {{}} double brackets (one for the object, one for the jsx expression)
- Attributes are in camelCase (following JSX notation)
- class -> className (add more classes, space separated)
- Treat the function as it returns its own html element. Return just the 'div' element (without separate closing tag) to be closed in use.
